...
public interface SoftwareRepository  extends JpaRepository<Software, Long>  {
    ...
    @Query("select new com.sysadminanywhere.inventory.controller.dto.SoftwareCount(s.id, s.name, s.vendor, s.version, count(i)) from Software s JOIN s.installations i GROUP BY s.id, s.name, s.vendor, s.version order by s.name")
    Page<SoftwareCount> getSoftwareInstallationCount(Pageable pageable);
